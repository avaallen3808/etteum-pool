...
export class DeepSeekWebProvider extends BaseProvider {
  name = "deepseek-web";
  supportedModels: ModelInfo[] = [
    { id: "deepseek-web", object: "model" as const, created: Date.now(), owned_by: "deepseek-web", context_window: 128000, max_output: 4000, thinking: false, vision: false, creditUnit: "token" as const, creditRate: 0, creditSource: "estimated" as const },
  ...
  private getCookie(a: Account): string { try{ const c=decrypt(a.password); if(c) return c; }catch{} const t=(a.tokens as unknown as Record<string,unknown>); if(t?.cookie) return String(t.cookie); if(typeof a.tokens==="string" && a.tokens.includes("userToken")) return a.tokens; return ""; }
  ...
  async chatCompletion(a: Account, r: ChatCompletionRequest): Promise<ProviderResult> {
    const c=this.getCookie(a); if(!c) return {success:false, error:"No DeepSeek cookie (paste userToken from chat.deepseek.com Local Storage)"};
    ...
    const res=await this.run({site:"deepseek", prompt, messages:r.messages, cookies:c},60000);
    if(!res.ok) return {success:false, error:res.error||"DeepSeek web failed"};
  ...
  async refreshToken(): Promise<{success:boolean; tokens?:string; error?:string}> { return {success:false, error:"DeepSeek web requires cookie refresh"}; }
  ...
  async fetchQuota(a: Account): Promise<{success:boolean; quota?:{limit:number; remaining:number; used:number; resetAt?:Date|string|null}; error?:string}> {
    ...
    const r=await this.run({site:"deepseek", mode:"quota", cookies:c},15000);
