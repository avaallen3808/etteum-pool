...
// NVIDIA NIM — 70+ models, OpenAI-compatible
// Base: https://integrate.api.nvidia.com/v1  Auth: Bearer nvapi-...
// Free: ~40 RPM, rate-limited dev access
...
const LIST: Def[] = [
  { id: "nvidia-llama-3.3-70b", upstream: "meta/llama-3.3-70b-instruct", context_window: 128000, max_output: 4000 },
  { id: "nvidia-llama-3.1-405b", upstream: "meta/llama-3.1-405b-instruct", context_window: 128000, max_output: 4000 },
  { id: "nvidia-deepseek-r1", upstream: "deepseek-ai/deepseek-r1", context_window: 128000, max_output: 8000 },
  { id: "nvidia-deepseek-v3.2", upstream: "deepseek-ai/deepseek-v3.2", context_window: 128000, max_output: 4000 },
  { id: "nvidia-qwen3-235b", upstream: "qwen/qwen3-235b-a22b", context_window: 128000, max_output: 4000 },
  { id: "nvidia-kimi-k2.5", upstream: "moonshotai/kimi-k2.5", context_window: 128000, max_output: 4000 },
  { id: "nvidia-glm-4.7", upstream: "zai/glm-4.7", context_window: 128000, max_output: 4000 },
  { id: "nvidia-nemotron-nano-9b", upstream: "nvidia/nemotron-nano-9b-v2", context_window: 128000, max_output: 4000 },
];
function norm(m: string): string {
  const l = m.toLowerCase().trim();
  if (l.startsWith("nvidia/")) return l.slice(7);
  if (l.startsWith("nvidia-")) return l.slice(7);
  if (l.startsWith("nv-")) return l.slice(3);
  return l;
}
const MAP: Record<string, Def> = Object.fromEntries(LIST.flatMap((d) => [[d.id.toLowerCase(), d], [norm(d.id), d], [`nvidia/${norm(d.id)}`, d], [`nv-${norm(d.id)}`, d], [d.upstream.toLowerCase(), d]]));
export class NvidiaProvider extends BaseProvider {
  ...
  supportedModels: ModelInfo[] = LIST.map((d) => ({ id: d.id, object: "model" as const, created: Date.now(), owned_by: "nvidia", context_window: d.context_window, max_output: d.max_output, thinking: false, vision: false, creditUnit: "token" as const, creditRate: 0, creditSource: "estimated" as const }));
  ...
  async chatCompletion(a: Account, r: ChatCompletionRequest): Promise<ProviderResult> { const d = this.resolve(r.model); if (!d) return { success: false, error: `Unknown Nvidia model: ${r.model}` }; return this.chat(a, d, r); }
  async chatCompletionStream(a: Account, r: ChatCompletionRequest): Promise<ProviderResult> { const d = this.resolve(r.model); if (!d) return { success: false, error: `Unknown Nvidia model: ${r.model}` }; return this.stream(a, d, r); }
  ...
  async fetchQuota(a: Account): Promise<{ success: boolean; quota?: { limit: number; remaining: number; used: number; resetAt?: Date | string | null }; error?: string }> {
    ...
    try { const r = await this.fetchWithTimeout(MODELS, { method: "GET", headers: { Authorization: `Bearer ${k}` } }); if (r.status === 401 || r.status === 403) return { success: false, error: `expired: HTTP ${r.status}` }; if (!r.ok) { const t = await r.text().catch(() => ""); return { success: false, error: `Nvidia HTTP ${r.status}: ${t.slice(0,160)}` }; } await r.text().catch(() => ""); return { success: true, quota: { limit: -1, remaining: -1, used: 0, resetAt: null } }; } catch (e) { return { success: false, error: e instanceof Error ? e.message : String(e) }; }
  }
  private async chat(a: Account, d: Def, r: ChatCompletionRequest): Promise<ProviderResult> {
    ...
    try { const resp = await this.fetchWithTimeout(CHAT, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${k}` }, body: JSON.stringify(b) }); const e = await this.err(resp, "Nvidia chat"); if (e) return e; const data = (await resp.json()) as ChatCompletionResponse; const ch = data.choices?.[0]; if (!ch) return { success: false, error: "No choices" }; const pt = data.usage?.prompt_tokens ?? this.estimateMessagesTokens(r.messages); const ct = data.usage?.completion_tokens ?? this.estimateTokens(typeof ch.message?.content === "string" ? ch.message.content : ""); data.model = r.model; return { success: true, response: data, promptTokens: pt, completionTokens: ct, tokensUsed: pt + ct }; } catch (e) { return { success: false, error: e instanceof Error ? e.message : String(e) }; }
  }
...
export async function activateNvidiaKey(apiKey: string): Promise<{ email: string; metadata: Record<string, unknown> }> {
  ...
  if (!r.ok) { const x = await r.text().catch(() => ""); throw new Error(`Nvidia HTTP ${r.status}: ${x.slice(0,200)}`); }
  await r.text().catch(() => "");
  const h = t.slice(-8).toLowerCase().replace(/[^a-z0-9]/g, "x");
  return { email: `nvidia-${h}@nvidia`, metadata: { validated_at: new Date().toISOString() } };
}
