...
export class ZaiWebProvider extends BaseProvider {
  name = "zai-web";
  ...
  async chatCompletion(a: Account, r: ChatCompletionRequest): Promise<ProviderResult> {
    const c=this.getCookie(a); if(!c) return {success:false, error:"No ZAI token (paste token from chat.z.ai Local Storage)"};
    ...
    const res=await this.run({site:"zai", prompt, messages:r.messages, cookies:c},60000);
    if(!res.ok) return {success:false, error:res.error||"ZAI web failed"};
  ...
  async chatCompletionStream(a: Account, r: ChatCompletionRequest): Promise<ProviderResult> {
    const c=this.getCookie(a); if(!c) return {success:false, error:"No ZAI token (paste token from chat.z.ai Local Storage)"};
    const last=[...r.messages].reverse().find((m)=>m.role==="user"); const prompt=typeof last?.content==="string"?last.content:JSON.stringify(last?.content||"");
    return this.webWorkerStream({ worker: WORKER, tmpPrefix: "zai", site: "zai", prompt, messages: r.messages, cookies: c, model: r.model, timeoutMs: 60000 });
  }
  async refreshToken(): Promise<{success:boolean; tokens?:string; error?:string}> { return {success:false, error:"ZAI web requires token refresh"}; }
  ...
  async fetchQuota(a: Account): Promise<{success:boolean; quota?:{limit:number; remaining:number; used:number; resetAt?:Date|string|null}; error?:string}> {
    ...
    const r=await this.run({site:"zai", mode:"quota", cookies:c},15000);
